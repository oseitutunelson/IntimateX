...
const NftFeed = () => {
  ...
  const fetchNftFeed = async () => {
    ...
    try {
      const response = await axios.get(
        ...
        { crossdomain: true, withCredentials: false , headers: {
  ...
  const handleVideoClick = async (nft) => {
    const {creator,ImgHash} = nft;
    // Increment View Count
    try {
      const response = await axios.post('http://localhost:5000/api/videos/view', { creator : creator,videoId: ImgHash });
            ...
            fetchedFeed.map((nft,index) =>
              async () => {
                try {
                  ...
                  const viewResponse = await axios.get(`http://localhost:5000/api/videos/${nft.ImgHash}`);
                  return { ...nft, hasAccess,views: viewResponse.data.views }; // Add views to NFT object
                } catch (viewError) {
                  console.error(`Error fetching views for ${nft.ImgHash}:`, viewError);
                  return { ...nft, views: 0 }; // Default to 0 views if error occurs
                }
              ...
              <div key={index} className="nft-card">
                {nft.price === 0 || nft.hasAccess  ? (
                  ...
                  >
                    ...
                    >
                      <source
                        src={`https://gateway.pinata.cloud/ipfs/${nft.ImgHash}`}
                        type="video/mp4"
                      />
                    </video>
